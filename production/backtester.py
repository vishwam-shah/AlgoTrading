...
class WalkForwardBacktester:
    ...
    def __init__(
        ...
        max_position_pct: float = 0.15,  # Max 15% per position (increased for conviction)
        stop_loss_atr_mult: float = 3.0,  # Stop loss at 3x ATR (wider to reduce whipsaws)
        take_profit_atr_mult: float = 5.0,  # Take profit at 5x ATR (better R:R ratio)
        max_holding_days: int = 30,  # Maximum holding period (longer trends)
        use_trailing_stop: bool = True,  # Enable trailing stops
        trailing_stop_activation: float = 0.03,  # Activate trailing after 3% profit
    ...
    def run_backtest(
        ...
        for i in range(len(data)):
            ...
            for symbol, trade in positions.items():
                ...
                if self.use_trailing_stop and trade.direction == 'LONG':
                    if high > trade.highest_price:
                        ...
                        if profit_pct >= self.trailing_stop_activation:
                            ...
                            trail_distance = max(self.trailing_stop_distance * 0.7, 0.01)  # Tighter trail for bigger profits
                ...
                # Check take profit (let winners run longer)
                ...
                # Trend breakdown exit - exit if price falls below 20-day SMA when in loss
                elif unrealized_pnl_pct < 0 and 'sma_20' in data.columns:
                    if current_price < data['sma_20'].iloc[i] * 0.98:  # 2% below SMA
                        trade.exit_price = current_price
                        trade.exit_reason = 'TREND_BREAKDOWN'
                        positions_to_close.append(symbol)

                # Check signal reversal (exit on reversal if not deeply in profit)
                elif predictions[i] != (1 if trade.direction == 'LONG' else 0):
                    # If in small profit or loss, exit on reversal
                    if unrealized_pnl_pct < 0.02:  # Not more than 2% profit
                        trade.exit_price = current_price
            ...
            if len(positions) == 0 and confidences[i] >= min_confidence:
                # Volume confirmation: only trade if volume > 1.1x 20-day average (relaxed)
                volume_confirmed = True
                if 'volume_sma_20' in data.columns:
                    avg_vol = data['volume_sma_20'].iloc[i]
                    if avg_vol > 0:
                        volume_ratio = data['volume'].iloc[i] / avg_vol
                        volume_confirmed = volume_ratio >= 1.0  # Relaxed - any average volume ok
                
                # Trend confirmation: price above 20-day SMA for buys (stronger trend)
                trend_confirmed = True
                if 'sma_20' in data.columns:
                    trend_confirmed = current_price > data['sma_20'].iloc[i]
                elif 'sma_10' in data.columns:
                    trend_confirmed = current_price > data['sma_10'].iloc[i]
                
                # Momentum filter: RSI not overbought (< 70 for buys)
                momentum_ok = True
                if 'rsi_14' in data.columns:
                    rsi = data['rsi_14'].iloc[i]
                    momentum_ok = 30 < rsi < 70  # Not oversold or overbought
                
                # MACD confirmation: MACD above signal line OR rising (bullish)
                macd_bullish = True
                if 'macd' in data.columns and 'macd_signal' in data.columns:
                    macd = data['macd'].iloc[i]
                    macd_signal = data['macd_signal'].iloc[i]
                    # Bullish if MACD above signal OR MACD is rising
                    if i > 0 and 'macd' in data.columns:
                        macd_prev = data['macd'].iloc[i-1]
                        macd_bullish = (macd > macd_signal) or (macd > macd_prev)
                    else:
                        macd_bullish = macd > macd_signal
                
                # Sentiment filter (if available): not strongly bearish
                sentiment_ok = True
                if 'news_sentiment' in data.columns:
                    sent = data['news_sentiment'].iloc[i]
                    sentiment_ok = sent >= -0.2 or pd.isna(sent) or sent == 0  # Not strongly bearish, accept neutral/unknown
                
                # Core conditions (must pass)
                core_conditions = predictions[i] == 1 and momentum_ok
                
                # Supporting conditions (count how many pass)
                ...
                # Enter if core conditions pass AND at least 2 supporting conditions
                if core_conditions and supporting_score >= 2:
                    ...
                    vol_adjustment = min(0.20 / (vol_20 + 0.01), 1.0)  # More conservative
    ...
    def _calculate_metrics(
        ...
        # Create equity series with date index from data
        ...
        # Align dates with equity (equity has one more element than data rows)
        ...
        losing_trades = len([t for t in trades if t.pnl < 0])  # Strictly negative
        win_rate = winning_trades / total_trades if total_trades > 0 else 0

        wins = [t.pnl for t in trades if t.pnl > 0]
        losses = [abs(t.pnl) for t in trades if t.pnl < 0]  # Use absolute values for losses

        avg_win = np.mean(wins) if wins else 0
        avg_loss = np.mean(losses) if losses else 0

        # Profit factor: total wins / total losses (if no losses, use infinity-like value)
        total_wins = sum(wins) if wins else 0
        total_losses = sum(losses) if losses else 0
        if total_losses > 0:
            profit_factor = total_wins / total_losses
        elif total_wins > 0:
            profit_factor = 99.99  # All wins, no losses - cap at reasonable max
        else:
            profit_factor = 0.0
