"""
================================================================================
TCN CLASSIFIER - Temporal Convolutional Network for Stock Prediction
================================================================================
State-of-the-art architecture using dilated causal convolutions.

Key Features:
- Parallel training (faster than LSTM)
- Captures very long-term dependencies
- Dilated convolutions for efficient receptive field
- Often outperforms LSTM on time series

Expected Performance: 61-69% accuracy ⭐
...
class TCNClassifier(BaseDeepLearningModel):
    """
    Temporal Convolutional Network for stock direction prediction.
    ...
    def __init__(
        ...
        Args:
            ...
            num_levels: Number of TCN levels (depth)
            ...
            device: 'cpu' or 'cuda'
        """
        super().__init__(model_name="TCN", seq_length=seq_length)
        ...
        try:
            ...
            class TCNNet(nn.Module):
                ...
                def __init__(self, input_size, num_filters, num_levels, kernel_size, dropout):
                    ...
                    # Initial projection: input_size -> num_filters
                    self.input_conv = nn.Conv1d(input_size, num_filters, 1)
                    ...
                    for level in range(num_levels):
                        ...
                        self.tcn_blocks.append(
                            ResidualBlock(num_filters, kernel_size, dilation, dropout)
                        )
...
if __name__ == '__main__':
    ...
    print(f"  Training complete: {history['final_epoch']} epochs")
    ...
    for i in range(min(5, len(preds))):
        y_idx = model.seq_length + i
        print(f"    {i:2d}   |     {preds[i]}      |   {proba[i]:.3f}    |  {y_test[y_idx]}")

    print("\n[SUCCESS] TCN Classifier working correctly!")
