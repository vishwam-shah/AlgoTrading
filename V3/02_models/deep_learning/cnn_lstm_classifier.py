...
================================================================================
CNNLSTMClassifier — Causal 1D-CNN feature extractor + LSTM temporal model.
...
Design Rationale
----------------
- padding='causal': Conv1D at step t only sees steps 0..t — zero future leakage
  through the convolution operation itself.
- Two Conv1D layers: first captures local 3-day candlestick/momentum patterns;
  second stacks those into higher-order week-level formations.
- BatchNorm after Conv1D only: stable with feature maps; intentionally omitted
  after LSTM (temporal BN is unstable with variable-length batches and
  sample_weight) and after Dense (small batch sizes make BN noisy).
- MaxPool(2) halves the sequence 20→10 before LSTM, reducing recurrent compute
  and forcing hierarchical temporal compression.
- LSTM(32): CNN has already extracted local features; the LSTM needs fewer
  units to model the compressed temporal dynamics.
================================================================================
"""
...
from config import DL_SEQ_LEN, CNN_LSTM_PARAMS           # noqa: E402
from deep_learning.base_deep import BaseDLClassifier     # noqa: E402


class CNNLSTMClassifier(BaseDLClassifier):
    """Causal 1D-CNN + LSTM binary classifier."""
    ...
    def __init__(
        ...
        self._lr = CNN_LSTM_PARAMS["learning_rate"]
    ...
    def _build(self):
        ...
        return keras.Model(inp, out, name="cnn_lstm_classifier")
