...
\textbf{Implication for V3.} The gate is set so that the model is only allowed to trade on days where calibrated probability exceeds $0.58$ \emph{and} the predicted absolute move exceeds the round-trip cost ($\sim 0.4\%$ for NSE delivery). The reported headline number on all days (51.45\%) is consistent with weak-form efficiency; the headline number on signal days (60.33\%) is the figure that matters operationally.
...
\midrule
V3 (this thesis)                       & 10-model + temp.\ cal.  & 99 NSE Nifty-100 & $\sim$200 & 6-fold walk-forward; 51.5\% (all) / \textbf{60.3\%} (gated) \\
\bottomrule
\end{tabular}\\[4pt]
